---
title: 'Game of Thrones sentiment analysis Mathias Dyhr'
date: 'created on 21 marts 2025 and updated `r format(Sys.time(), "%d %B, %Y")`'
...
Let's count the words! sorting from most frequent to lesser frequent
```{r count-words}
got_wc <- got_tokens %>% 
  count(word) %>% 
  arrange(-n)
...
ggplot(data = got_top100, aes(label = word, size = n)) +
  geom_text_wordcloud_area(aes(color = n), shape = "cirkel") +
  scale_size_area(max_size = 15) +
  scale_color_gradientn(colors = c("purple","orange","blue")) +
  theme_minimal()
```
